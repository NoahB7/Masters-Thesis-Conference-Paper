...
class GaussianDiffusion(nn.Module):
    def __init__(self, mean_type, noise_schedule, noise_scale, noise_min, noise_max,\
            steps, device, p_uncond, history_num_per_term=10, beta_fixed=True):
    ...
    # used for inference or actual calls, calls q_sample then model call to perform both noising and denoising for oyu
    def p_sample(self, model, x_start, steps, sampling_noise=False):
        ...
        if self.noise_scale == 0.:
            for i in indices:
                ...
                x_t = model(x_t, t, x_start, self.p_uncond)
            return x_t

        for i in indices:
            t = th.tensor([i] * x_t.shape[0]).to(x_start.device)
            out = self.p_mean_variance(model, x_t, t, x_start)
            if sampling_noise:
    ...
    def training_losses(self, model, x_start, reweight=False):
        ...
        terms = {}
        model_output = model(x_t, ts, x_start, self.p_uncond)
        target = {
    ...
    def p_mean_variance(self, model, x, t, x_start):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B, )
        model_output = model(x, t, x_start, self.p_uncond)
